...
const splitDateIntern = (data: InternScheduleData[]) => {
    ...
    data.map((item)=> {
        const [itemday, itemmonth, itemyear] = item.register_date.split('-').map(Number);
        result.push({ register_day: itemday, register_month: itemmonth, register_year: itemyear, 
            register_shift: item.register_shift, start_time: item.start_time, end_time: item.end_time, add: false})
    })
    return result;
  }

export const getInternSchedule = async (month: number, year: number, dataInternAPI: InternScheduleData[]) => {
    let data : InternSchedule = { month: month, year: year, schedule: [] };
    const dataIntern = splitDateIntern(dataInternAPI);
    const dataInternMonth = dataIntern.filter(y => y.register_year === year && y.register_month === month);
    const day = 1;
    let i = 0;
    while(true){
        ...
        if ((SundayofWeek.getFullYear() > year && date.getFullYear() > year) 
          || (SundayofWeek.getMonth() > month - 1 && date.getMonth() > month - 1)){break; }
...
const getInternScheduleWeek = (startDate: Date, data: InternScheduleDay[]) => {
    let dataweek: InternScheduleWeek = {
        T2: { register_day: 0, register_month: 0, register_year: 0, 
            register_shift: "", start_time: "", end_time: "", add: false},
        T3: { register_day: 0, register_month: 0, register_year: 0, 
            register_shift: "", start_time: "", end_time: "", add: false},
        T4: { register_day: 0, register_month: 0, register_year: 0, 
            register_shift: "", start_time: "", end_time: "", add: false},
        T5: { register_day: 0, register_month: 0, register_year: 0, 
            register_shift: "", start_time: "", end_time: "", add: false},
        T6: { register_day: 0, register_month: 0, register_year: 0, 
            register_shift: "", start_time: "", end_time: "", add: false},
        T7: { register_day: 0, register_month: 0, register_year: 0, 
            register_shift: "", start_time: "", end_time: "", add: false},
        CN: { register_day: 0, register_month: 0, register_year: 0, 
            register_shift: "", start_time: "", end_time: "", add: false},
    };
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const internDay = data.filter(y => y.register_day === newday);
