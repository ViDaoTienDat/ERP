...
export const handleSplitHisCheckIn = async (data: HisCheckIn[]):Promise<DateHisCheckIn[]> => {
        ...
        if (yearData) {
          ...
          if (monthData) {
            ...
            if (dayData) {
              ...
              monthData.datamonth.push({ day: itemday, data: [`${hour}:${minute}`] });
            }
          } else {
            yearData.datayear.push({ month: itemmonth, datamonth: [{ day: itemday, data: [`${hour}:${minute}`] }] });
          }
        } else {
          result.push({ 
            year: itemyear, 
            datayear: [{ month: itemmonth, datamonth: [{ day: itemday, data: [`${hour}:${minute}`] }] }] 
          });
        }
      });
    return result;
...
export const getCalendarCheckIn = async (month: number, year: number, dateHisCheckIn: DateHisCheckIn[]) => {
...
const getWeekCheckIn = (date: Date, dataYearHis: DateHisCheckIn | undefined ) => {
