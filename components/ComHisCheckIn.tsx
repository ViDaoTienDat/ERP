...
export function RowCalendar({data, month, year}: DataJson): React.JSX.Element {
    return(
  ...
  function CellCalendar({data, month, year}: DataJson): React.JSX.Element {
    return(
        <View style={selfstyle.cellcalendar}>
            <Text style={[selfstyle.text_small, 
                (data.day == day && data.month==currmonth &&year==curryear)?{color: Color.color3}:data.month!=month?{color: '#ccc'}: {}]}
            >
                {data.day}
            </Text>
            <View style={selfstyle.boxcheck}>
                <Text style={selfstyle.text_check}>{data.checkin? data.checkin: ''}</Text>
                <Text style={selfstyle.text_check}>{data.checkout? data.checkout: ''}</Text>
            </View>
        </View>
    )
  }
